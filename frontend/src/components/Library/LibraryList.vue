            ...
            <li v-for="book in books" :key="book.id" class="h-auto inline-block w-[49%] m-2">
                <button type="button" @click="startModal(book)" class="transition duration-300 ease-in-out h-full w-full shadow-lg bg-white rounded-xl py-4 px-5 
                    ...
                    <div class="w-full h-full flex flex-col gap-1">
                        ...
                        <GenresAuthors IfTitle="Жанры" ElseTitle="Жанр" :list="book.genres" classStyle="bg-green-500 w-auto p-[5px] rounded" />
                        <GenresAuthors IfTitle="Авторы" ElseTitle="Автор" :list="book.authors" classStyle="bg-blue-500 w-auto p-[5px] rounded" />
                    </div>
                </button>
            </li>
        </ul>
